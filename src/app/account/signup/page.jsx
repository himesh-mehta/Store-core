...
export default function SignupPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try { 
      ...
      const c = e?.code;
      setError(
        c === "auth/email-already-in-use" ? "This email is already registered."
  ...
  return (
    <div className="min-h-screen bg-white flex" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 bg-[#111] p-12">
        ...
        <div>
          <p className="text-[9px] font-black tracking-[0.3em] uppercase text-[#888] mb-5">Join us</p>
          <h2 className="text-5xl font-black tracking-tighter leading-none text-white mb-6">
            Create<br />
            <span className="font-thin text-[#888]">your account.</span>
          </h2>
          <p className="text-[#888] text-sm leading-relaxed max-w-xs">
            Free forever. Access 15+ premium products, secure checkout, and order tracking.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-black mb-10">Get started</h1>
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            <div>
              <label className="block text-[10px] font-black tracking-[0.2em] uppercase text-[#888] mb-1.5">Full name</label>
              <input required type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" className={inputCls} />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input required type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 6 characters" className={`${inputCls} pr-12`} />
            ...
            <button type="submit" disabled={loading}
              ...
              {loading
                ...
                : (<>Create Account <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" /></>)
              }
            </button>
          </form>
          ...
          <p className="mt-8 text-[12px] text-[#888]">
            ...
            <a href="/account/signin" className="font-bold text-black hover:text-[#444] transition-colors underline underline-offset-2">
              Sign in →
            </a>
          </p>
        </div>
